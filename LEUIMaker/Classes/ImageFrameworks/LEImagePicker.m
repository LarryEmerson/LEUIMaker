...
-(id) initWithCollection:(PHAssetCollection *) collection Remain:(NSInteger) remain Max:(NSInteger) max Assets:(NSArray<PHAsset *> *)assets Delegate:(id<LEImagePickerDelegate>) delegate{
    ...
    collectionView=[LECollectionView alloc].leSuperview(view.leSubViewContainer, layout, @"LEMultiImagePickerItem").leDelegate(self);
                ...
                [PHPhotoLibrary requestAuthorization:^(PHAuthorizationStatus status) {
                    if(status==PHAuthorizationStatusAuthorized){
                        dispatch_sync(dispatch_get_main_queue(), ^{
                            [LEImagePicker new].leSuperViewcontroller(weakself.leVC,weakself.leDelegate).leRemainCount(remain).leMaxCount(max).leSelectedAssets(assets);
                        });
                    }
                }];

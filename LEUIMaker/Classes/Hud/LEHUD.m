...
+(void) leShowHud:(NSString *) text Offset:(CGFloat) offset EnterTime:(float) time PauseTime:(float) pauseTime ReleaseWhenFinished:(BOOL) isRealse{
    LEHUD *noti=[LEHUD new].leAddTo([LEUICommon sharedInstance].leGetTopWindow).leAnchor(LEI_BC).leBottom(LEStatusBarHeight).leWidth(LESCREEN_WIDTH-LENavigationBarHeight).leHeight(LENavigationBarHeight);

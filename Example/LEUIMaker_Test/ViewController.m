...
//    view=[UIView new].leAddTo(self.view).leAnchor(LEI_TC).leWidth(100).leHeightEqualWidth(1).leBgColor([UIColor darkGrayColor]);
//    [UIButton new].leAddTo(self.view).leAnchor(LEI_TL).leWidth(50).leHeightEqualWidth(1).leBgColor([UIColor yellowColor]).leTouchEvent(@selector(onAdd),self);
//    [UIButton new].leAddTo(self.view).leAnchor(LEI_TR).leWidth(50).leHeightEqualWidth(1).leBgColor([UIColor yellowColor]).leTouchEvent(@selector(onRemove),self);
//    return;
    
    [self.view setBackgroundColor:[UIColor redColor]];
#pragma mark 新建一个ScrollView，用于测试自动计算内部组件的排版，设置ContentSize，通过（leAutoResizeContentView）来完成，无需后期再关注Contentsize的计算
    scrollView=[UIScrollView new].leAddTo(self.view).leEqualSuperViewWidth(1).leEqualSuperViewHeight(1).leAutoResizeContentView().leBgColor([UIColor colorWithRed:184/255.0 green:184/255.0 blue:184/255.0 alpha:1]); 
#pragma mark 初始配置
